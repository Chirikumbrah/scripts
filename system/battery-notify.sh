...
while true; do
    ...
    if [[ "$BATTERY_STATE" = "pending-charge" ]]; then
        ...
        dunstify -t 7000 -i /usr/share/icons/ePapirus/32x32/devices/battery.svg "Low battery level ($BATTERY_POWER%)" "Please plug the charger"
        sleep 2m
    elif [[ "$BATTERY_STATE" = "discharging" && "$BATTERY_POWER" -le 10 && "$BATTERY_POWER" -ge 3 ]]; then
        dunstify -t 7000 -i /usr/share/icons/ePapirus/32x32/devices/battery.svg -u critical "Critical battery level ($BATTERY_POWER%)" "Plug the charger immediately!"
        sleep 1m
    elif [[ "$BATTERY_STATE" = "discharging" && "$BATTERY_POWER" -eq 1 ]]; then
        dunstify -t 7000 -i /usr/share/icons/ePapirus/32x32/devices/battery.svg -u critical "Battery almost is empty ($BATTERY_POWER%)!!!" "HEEEEEEEEEEEELP MEEE!!!!" 
        sleep 11
    else
        sleep 5m
    fi
